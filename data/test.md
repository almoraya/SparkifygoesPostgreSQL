...
- [Architecture Diagrams](#archdiagrams)
- [Tips](#tips)
<br>
<br>
<br>
...
It is important to follow a specific order when deploying an app to kubernetes. The following order is the one who worked the best for me.
`alias k=kubectl`

1. `k apply -f deployment/db-configmap.yaml` - Set up environment variables for the pods
2. `k apply -f deployment/db-secret.yaml` - Set up secrets for the pods
3. `k apply -f deployment/postgres.yaml` - Set up a Postgres database running PostGIS


4. `k apply -f deployment/uda-api-person-connection.yaml` - Set up the service and deployment for the person-connection API
5. `k apply -f deployment/uda-api-persons` - Set up the service and deployment for the persons API
6. `k apply -f deployment/uda-api-location.yaml` - Set up the service and deployment for the location API
...
* `http://localhost:30004/` - OpenAPI Person Connection Documentation
* `http://localhost:30008/` - OpenAPI Persons Documentation
* `http://localhost:300010/` - OpenAPI Locations Documentation
* `http://localhost:30004/api/` - Base path for API Person Connection
* `http://localhost:30008/api/` - Base path for API Persons
* `http://localhost:30010/api/` - Base path for API Locations
...
Below the same architectural design by from a port point of view
<td><img  width="45%" height= "45%" src=docs/architecture_design1.png alt="Architecture Design 1"></td>
<td><img  width="45%" height= "45%" src=docs/architecture_design2.png alt="Architecture Design 2"></td>
<br>
<br>
...
<p align="center">
<img  width="65%" height= "50%" src=docs/kafka_instructions.png alt="Running services">
</p>
